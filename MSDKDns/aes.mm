...
* Details:    This code is the implementation of the AES algorithm and
               ...
               *
http://csrc.nist.gov/publications/nistpubs/800-38a/sp800-38a.pdf . The CCM mode
of operation is specified by NIST SP80-38 C, available at: *
...
static void CcmPrepareFirstCtrBlk(BYTE *counter, const BYTE *nonce,
                                  int nonce_len, int payload_len_store_size);

static void CcmPrepareFirstFormatBlk(BYTE *buf, int assoc_len, int payload_len,
                                     int payload_len_store_size, int mac_len,
                                     const BYTE *nonce, int nonce_len);

static void CcmFormatAssocData(BYTE *buf, int *end_of_buf, const BYTE *assoc,
                               int assoc_len);

static void CcmFormatPayloadData(BYTE *buf, int *end_of_buf,
...
// index (column).
static const BYTE aesSbox_[16][16] = {
...
static const BYTE aesInvsbox_[16][16] = {
...
static const BYTE gfMul_[256][6] = {
...
static void XorBuf(const BYTE *in, BYTE *out, size_t len) {
  size_t idx;
 ...
 * AES - CBC
 *******************/
int AesEncryptCbc(const unsigned char *in, size_t in_len, unsigned char *out,
  ...
  for (idx = 0; idx < blocks; idx++) {
    ...
    XorBuf(iv_buf, buf_in, AES_BLOCK_SIZE);
    AesEncrypt(buf_in, buf_out, key, keysize);
...
int AesEncryptCbcMac(const BYTE *in, size_t in_len, unsigned char *out,
...
int AesDecryptCbc(const unsigned char *in, size_t in_len, unsigned char *out,
  ...
  for (idx = 0; idx < blocks; idx++) {
    memcpy(buf_in, &in[idx * AES_BLOCK_SIZE], AES_BLOCK_SIZE);
    AesDecrypt(buf_in, buf_out, key, keysize);
    XorBuf(iv_buf, buf_out, AES_BLOCK_SIZE);
    memcpy(&out[idx * AES_BLOCK_SIZE], buf_out, AES_BLOCK_SIZE);
 ...
 * AES - CTR
 *******************/
void IncrementIv(BYTE *iv, int counter_size) {
  int idx;
...
void AesEncryptCtr(const BYTE *in, size_t in_len, BYTE *out, const WORD *key,
  ...
  if (in_len > AES_BLOCK_SIZE) {
    for (idx = 0; idx < last_block_length; idx += AES_BLOCK_SIZE) {
      ...
      XorBuf(out_buf, &out[idx], AES_BLOCK_SIZE);
      IncrementIv(iv_buf, AES_BLOCK_SIZE);
    }
  }

  AesEncrypt(iv_buf, out_buf, key, keysize);
  XorBuf(out_buf, &out[idx], in_len - idx);  // Use the Most Significant bytes.
}

void AesDecryptCtr(const BYTE *in, size_t in_len, BYTE *out, const WORD *key,
                   int keysize, const BYTE *iv) {
  // CTR encryption is its own inverse function.
  AesEncryptCtr(in, in_len, out, key, keysize, iv);
}
...
int AesEncryptCcm(const BYTE *payload, WORD payload_len, const BYTE *assoc,
                  uint16_t assoc_len, const BYTE *nonce, uint16_t nonce_len,
                  BYTE *out, WORD *out_len, WORD mac_len, const BYTE *key_str,
                  int keysize) {
  ...
  if (assoc_len > 32768 /* = 2^15 */) return (FALSE);

  buf = static_cast<BYTE *>(malloc(
                              payload_len + assoc_len +
                              48 /*Round both payload and associated data up a block size and add an extra block.*/));

  if (!buf) return (FALSE);

  // Prepare the key for usage.
  AesKeySetup(key_str, key, keysize);

  // Format the first block of the formatted data.
  payload_len_store_size = AES_BLOCK_SIZE - 1 - nonce_len;
  CcmPrepareFirstFormatBlk(buf, assoc_len, payload_len, payload_len_store_size,
                           mac_len, nonce, nonce_len);
  end_of_buf = AES_BLOCK_SIZE;

  // Format the Associated Data, aka, assoc[].
  CcmFormatAssocData(buf, &end_of_buf, assoc, assoc_len);

  // Format the Payload, aka payload[].
  CcmFormatPayloadData(buf, &end_of_buf, payload, payload_len);

  // Create the first counter block.
  CcmPrepareFirstCtrBlk(counter, nonce, nonce_len, payload_len_store_size);

  // Perform the CBC operation with an IV of zeros on the formatted buffer to
  // calculate the MAC.
  memset(temp_iv, 0, AES_BLOCK_SIZE);
  AesEncryptCbcMac(buf, (size_t)end_of_buf, mac, key, keysize, temp_iv);
  ...
  IncrementIv(temp_iv,
              AES_BLOCK_SIZE - 1 -
                  mac_len);  // Last argument is the byte size of the counting
  // portion of the counter block. /*BUG?*/
  AesEncryptCtr(out, payload_len, out, key, keysize, temp_iv);

  // Encrypt the MAC with CTR mode with a counter starting at 0.
  AesEncryptCtr(&out[payload_len], mac_len, &out[payload_len], key, keysize,
                counter);
...
int AesDecryptCcm(const BYTE ciphertext[], WORD ciphertext_len,
                  const BYTE assoc[], uint16_t assoc_len, const BYTE nonce[],
                  uint16_t nonce_len, BYTE plaintext[], WORD *plaintext_len,
                  WORD mac_len, int *mac_auth, const BYTE key_str[],
                  int keysize) {
  BYTE temp_iv[AES_BLOCK_SIZE];
  BYTE counter[AES_BLOCK_SIZE];
  BYTE mac[16];
  ...
  BYTE *buf;
  int end_of_buf;
  int plaintext_len_store_size;
  WORD key[60];

  if (ciphertext_len <= mac_len) return (FALSE);

  buf = static_cast<BYTE *>(
      malloc(assoc_len +
             ciphertext_len /*ciphertext_len = plaintext_len + mac_len*/ + 48));
  ...
  *plaintext_len = ciphertext_len - mac_len;
  ...
  CcmPrepareFirstCtrBlk(counter, nonce, nonce_len, plaintext_len_store_size);

  // Decrypt the Payload with CTR mode with a counter starting at 1.
  memcpy(temp_iv, counter, AES_BLOCK_SIZE);
  IncrementIv(temp_iv,
              AES_BLOCK_SIZE - 1 -
                  mac_len);  // (AES_BLOCK_SIZE - 1 - mac_len) is the byte size
  // of the counting portion of the counter block.
  AesDecryptCtr(plaintext, *plaintext_len, plaintext, key, keysize, temp_iv);

  // Setting mac_auth to NULL disables the authentication check.
  if (mac_auth != nullptr) {
    // Decrypt the MAC with CTR mode with a counter starting at 0.
    AesDecryptCtr(mac, mac_len, mac, key, keysize, counter);

    // Format the first block of the formatted data.
    plaintext_len_store_size = AES_BLOCK_SIZE - 1 - nonce_len;
    CcmPrepareFirstFormatBlk(buf, assoc_len, *plaintext_len,
                             plaintext_len_store_size, mac_len, nonce,
                             nonce_len);
    end_of_buf = AES_BLOCK_SIZE;
    ...
    CcmFormatPayloadData(buf, &end_of_buf, plaintext, *plaintext_len);

    // Perform the CBC operation with an IV of zeros on the formatted buffer to
    // calculate the MAC.
    memset(temp_iv, 0, AES_BLOCK_SIZE);
    AesEncryptCbcMac(buf, (size_t)end_of_buf, mac_buf, key, keysize, temp_iv);

    // Compare the calculated MAC against the MAC embedded in the ciphertext to
...
// Creates the first counter block. First byte is flags, then the nonce, then
// the incremented part.
static void CcmPrepareFirstCtrBlk(BYTE *counter, const BYTE *nonce,
                                  int nonce_len, int payload_len_store_size) {
  memset(counter, 0, AES_BLOCK_SIZE);
  counter[0] = (payload_len_store_size - 1) & 0x07;
  memcpy(&counter[1], nonce, (size_t)nonce_len);
}
...
static void CcmPrepareFirstFormatBlk(BYTE *buf, int assoc_len, int payload_len,
  ...
  buf[0] = (BYTE)(((((mac_len - 2) / 2) & 0x07) << 3) |
                  ((payload_len_store_size - 1) & 0x07));

  if (assoc_len > 0) buf[0] += 0x40;

  // Format the rest of the first block, storing the nonce and the size of the
  // payload.
  memcpy(&buf[1], nonce, (size_t)nonce_len);
  memset(&buf[1 + nonce_len], 0, (size_t)(AES_BLOCK_SIZE - 1 - nonce_len));
  buf[15] = (BYTE)(payload_len & 0x000000FF);
  buf[14] = (BYTE)((payload_len >> 8) & 0x000000FF);
}
...
static void CcmFormatAssocData(BYTE *buf, int *end_of_buf, const BYTE *assoc,
  ...
  buf[*end_of_buf + 1] = (BYTE)(assoc_len & 0x00FF);
  buf[*end_of_buf] = (BYTE)((assoc_len >> 8) & 0x00FF);
  *end_of_buf += 2;
  memcpy(&buf[*end_of_buf], assoc, (size_t)assoc_len);
  *end_of_buf += assoc_len;
  pad = AES_BLOCK_SIZE - (*end_of_buf % AES_BLOCK_SIZE); /*BUG?*/
  memset(&buf[*end_of_buf], 0, (size_t)pad);
  *end_of_buf += pad;
}
...
static void CcmFormatPayloadData(BYTE *buf, int *end_of_buf,
  ...
  memcpy(&buf[*end_of_buf], payload, (size_t)payload_len);
  *end_of_buf += payload_len;
  pad = *end_of_buf % AES_BLOCK_SIZE;

  if (pad != 0) pad = AES_BLOCK_SIZE - pad;
...
static WORD SubWord(WORD word) {
  ...
  result =
      static_cast<int>(aesSbox_[(word >> 4) & 0x0000000F][word & 0x0000000F]);
  result += static_cast<int>(
                aesSbox_[(word >> 12) & 0x0000000F][(word >> 8) & 0x0000000F])
            << 8;
  result += static_cast<int>(
                aesSbox_[(word >> 20) & 0x0000000F][(word >> 16) & 0x0000000F])
            << 16;
  result += static_cast<int>(
                aesSbox_[(word >> 28) & 0x0000000F][(word >> 24) & 0x0000000F])
            << 24;
  return (result);
}
...
void AesKeySetup(const BYTE *key, WORD *w, int keysize) {
  int Nb = 4;
  int Nr;
  int Nk;
  int idx;
...
static void SubBytes(BYTE state[][4]) {
  state[0][0] = aesSbox_[state[0][0] >> 4][state[0][0] & 0x0F];
  state[0][1] = aesSbox_[state[0][1] >> 4][state[0][1] & 0x0F];
  state[0][2] = aesSbox_[state[0][2] >> 4][state[0][2] & 0x0F];
  state[0][3] = aesSbox_[state[0][3] >> 4][state[0][3] & 0x0F];
  state[1][0] = aesSbox_[state[1][0] >> 4][state[1][0] & 0x0F];
  state[1][1] = aesSbox_[state[1][1] >> 4][state[1][1] & 0x0F];
  state[1][2] = aesSbox_[state[1][2] >> 4][state[1][2] & 0x0F];
  state[1][3] = aesSbox_[state[1][3] >> 4][state[1][3] & 0x0F];
  state[2][0] = aesSbox_[state[2][0] >> 4][state[2][0] & 0x0F];
  state[2][1] = aesSbox_[state[2][1] >> 4][state[2][1] & 0x0F];
  state[2][2] = aesSbox_[state[2][2] >> 4][state[2][2] & 0x0F];
  state[2][3] = aesSbox_[state[2][3] >> 4][state[2][3] & 0x0F];
  state[3][0] = aesSbox_[state[3][0] >> 4][state[3][0] & 0x0F];
  state[3][1] = aesSbox_[state[3][1] >> 4][state[3][1] & 0x0F];
  state[3][2] = aesSbox_[state[3][2] >> 4][state[3][2] & 0x0F];
  state[3][3] = aesSbox_[state[3][3] >> 4][state[3][3] & 0x0F];
}

static void InvSubBytes(BYTE state[][4]) {
  state[0][0] = aesInvsbox_[state[0][0] >> 4][state[0][0] & 0x0F];
  state[0][1] = aesInvsbox_[state[0][1] >> 4][state[0][1] & 0x0F];
  state[0][2] = aesInvsbox_[state[0][2] >> 4][state[0][2] & 0x0F];
  state[0][3] = aesInvsbox_[state[0][3] >> 4][state[0][3] & 0x0F];
  state[1][0] = aesInvsbox_[state[1][0] >> 4][state[1][0] & 0x0F];
  state[1][1] = aesInvsbox_[state[1][1] >> 4][state[1][1] & 0x0F];
  state[1][2] = aesInvsbox_[state[1][2] >> 4][state[1][2] & 0x0F];
  state[1][3] = aesInvsbox_[state[1][3] >> 4][state[1][3] & 0x0F];
  state[2][0] = aesInvsbox_[state[2][0] >> 4][state[2][0] & 0x0F];
  state[2][1] = aesInvsbox_[state[2][1] >> 4][state[2][1] & 0x0F];
  state[2][2] = aesInvsbox_[state[2][2] >> 4][state[2][2] & 0x0F];
  state[2][3] = aesInvsbox_[state[2][3] >> 4][state[2][3] & 0x0F];
  state[3][0] = aesInvsbox_[state[3][0] >> 4][state[3][0] & 0x0F];
  state[3][1] = aesInvsbox_[state[3][1] >> 4][state[3][1] & 0x0F];
  state[3][2] = aesInvsbox_[state[3][2] >> 4][state[3][2] & 0x0F];
  state[3][3] = aesInvsbox_[state[3][3] >> 4][state[3][3] & 0x0F];
}
...
static void MixColumns(BYTE state[][4]) {
  ...
  col[2] = state[2][0];
  col[3] = state[3][0];
  state[0][0] = gfMul_[col[0]][0];
  state[0][0] ^= gfMul_[col[1]][1];
  state[0][0] ^= col[2];
  state[0][0] ^= col[3];
  state[1][0] = col[0];
  state[1][0] ^= gfMul_[col[1]][0];
  state[1][0] ^= gfMul_[col[2]][1];
  state[1][0] ^= col[3];
  state[2][0] = col[0];
  state[2][0] ^= col[1];
  state[2][0] ^= gfMul_[col[2]][0];
  state[2][0] ^= gfMul_[col[3]][1];
  state[3][0] = gfMul_[col[0]][1];
  state[3][0] ^= col[1];
  state[3][0] ^= col[2];
  state[3][0] ^= gfMul_[col[3]][0];
  // Column 2
  col[0] = state[0][1];
  col[1] = state[1][1];
  col[2] = state[2][1];
  col[3] = state[3][1];
  state[0][1] = gfMul_[col[0]][0];
  state[0][1] ^= gfMul_[col[1]][1];
  state[0][1] ^= col[2];
  state[0][1] ^= col[3];
  state[1][1] = col[0];
  state[1][1] ^= gfMul_[col[1]][0];
  state[1][1] ^= gfMul_[col[2]][1];
  state[1][1] ^= col[3];
  state[2][1] = col[0];
  state[2][1] ^= col[1];
  state[2][1] ^= gfMul_[col[2]][0];
  state[2][1] ^= gfMul_[col[3]][1];
  state[3][1] = gfMul_[col[0]][1];
  state[3][1] ^= col[1];
  state[3][1] ^= col[2];
  state[3][1] ^= gfMul_[col[3]][0];
  // Column 3
  col[0] = state[0][2];
  col[1] = state[1][2];
  col[2] = state[2][2];
  col[3] = state[3][2];
  state[0][2] = gfMul_[col[0]][0];
  state[0][2] ^= gfMul_[col[1]][1];
  state[0][2] ^= col[2];
  state[0][2] ^= col[3];
  state[1][2] = col[0];
  state[1][2] ^= gfMul_[col[1]][0];
  state[1][2] ^= gfMul_[col[2]][1];
  state[1][2] ^= col[3];
  state[2][2] = col[0];
  state[2][2] ^= col[1];
  state[2][2] ^= gfMul_[col[2]][0];
  state[2][2] ^= gfMul_[col[3]][1];
  state[3][2] = gfMul_[col[0]][1];
  state[3][2] ^= col[1];
  state[3][2] ^= col[2];
  state[3][2] ^= gfMul_[col[3]][0];
  // Column 4
  col[0] = state[0][3];
  col[1] = state[1][3];
  col[2] = state[2][3];
  col[3] = state[3][3];
  state[0][3] = gfMul_[col[0]][0];
  state[0][3] ^= gfMul_[col[1]][1];
  state[0][3] ^= col[2];
  state[0][3] ^= col[3];
  state[1][3] = col[0];
  state[1][3] ^= gfMul_[col[1]][0];
  state[1][3] ^= gfMul_[col[2]][1];
  state[1][3] ^= col[3];
  state[2][3] = col[0];
  state[2][3] ^= col[1];
  state[2][3] ^= gfMul_[col[2]][0];
  state[2][3] ^= gfMul_[col[3]][1];
  state[3][3] = gfMul_[col[0]][1];
  state[3][3] ^= col[1];
  state[3][3] ^= col[2];
  state[3][3] ^= gfMul_[col[3]][0];
}
...
void InvMixColumns(BYTE state[][4]) {
  ...
  col[2] = state[2][0];
  col[3] = state[3][0];
  state[0][0] = gfMul_[col[0]][5];
  state[0][0] ^= gfMul_[col[1]][3];
  state[0][0] ^= gfMul_[col[2]][4];
  state[0][0] ^= gfMul_[col[3]][2];
  state[1][0] = gfMul_[col[0]][2];
  state[1][0] ^= gfMul_[col[1]][5];
  state[1][0] ^= gfMul_[col[2]][3];
  state[1][0] ^= gfMul_[col[3]][4];
  state[2][0] = gfMul_[col[0]][4];
  state[2][0] ^= gfMul_[col[1]][2];
  state[2][0] ^= gfMul_[col[2]][5];
  state[2][0] ^= gfMul_[col[3]][3];
  state[3][0] = gfMul_[col[0]][3];
  state[3][0] ^= gfMul_[col[1]][4];
  state[3][0] ^= gfMul_[col[2]][2];
  state[3][0] ^= gfMul_[col[3]][5];
  // Column 2
  col[0] = state[0][1];
  col[1] = state[1][1];
  col[2] = state[2][1];
  col[3] = state[3][1];
  state[0][1] = gfMul_[col[0]][5];
  state[0][1] ^= gfMul_[col[1]][3];
  state[0][1] ^= gfMul_[col[2]][4];
  state[0][1] ^= gfMul_[col[3]][2];
  state[1][1] = gfMul_[col[0]][2];
  state[1][1] ^= gfMul_[col[1]][5];
  state[1][1] ^= gfMul_[col[2]][3];
  state[1][1] ^= gfMul_[col[3]][4];
  state[2][1] = gfMul_[col[0]][4];
  state[2][1] ^= gfMul_[col[1]][2];
  state[2][1] ^= gfMul_[col[2]][5];
  state[2][1] ^= gfMul_[col[3]][3];
  state[3][1] = gfMul_[col[0]][3];
  state[3][1] ^= gfMul_[col[1]][4];
  state[3][1] ^= gfMul_[col[2]][2];
  state[3][1] ^= gfMul_[col[3]][5];
  // Column 3
  col[0] = state[0][2];
  col[1] = state[1][2];
  col[2] = state[2][2];
  col[3] = state[3][2];
  state[0][2] = gfMul_[col[0]][5];
  state[0][2] ^= gfMul_[col[1]][3];
  state[0][2] ^= gfMul_[col[2]][4];
  state[0][2] ^= gfMul_[col[3]][2];
  state[1][2] = gfMul_[col[0]][2];
  state[1][2] ^= gfMul_[col[1]][5];
  state[1][2] ^= gfMul_[col[2]][3];
  state[1][2] ^= gfMul_[col[3]][4];
  state[2][2] = gfMul_[col[0]][4];
  state[2][2] ^= gfMul_[col[1]][2];
  state[2][2] ^= gfMul_[col[2]][5];
  state[2][2] ^= gfMul_[col[3]][3];
  state[3][2] = gfMul_[col[0]][3];
  state[3][2] ^= gfMul_[col[1]][4];
  state[3][2] ^= gfMul_[col[2]][2];
  state[3][2] ^= gfMul_[col[3]][5];
  // Column 4
  col[0] = state[0][3];
  col[1] = state[1][3];
  col[2] = state[2][3];
  col[3] = state[3][3];
  state[0][3] = gfMul_[col[0]][5];
  state[0][3] ^= gfMul_[col[1]][3];
  state[0][3] ^= gfMul_[col[2]][4];
  state[0][3] ^= gfMul_[col[3]][2];
  state[1][3] = gfMul_[col[0]][2];
  state[1][3] ^= gfMul_[col[1]][5];
  state[1][3] ^= gfMul_[col[2]][3];
  state[1][3] ^= gfMul_[col[3]][4];
  state[2][3] = gfMul_[col[0]][4];
  state[2][3] ^= gfMul_[col[1]][2];
  state[2][3] ^= gfMul_[col[2]][5];
  state[2][3] ^= gfMul_[col[3]][3];
  state[3][3] = gfMul_[col[0]][3];
  state[3][3] ^= gfMul_[col[1]][4];
  state[3][3] ^= gfMul_[col[2]][2];
  state[3][3] ^= gfMul_[col[3]][5];
}
...
void AesEncrypt(const BYTE *in, BYTE *out, const WORD *key, int keysize) {
  BYTE state[4][4];
...
void AesDecrypt(const BYTE *in, BYTE *out, const WORD *key, int keysize) {
  BYTE state[4][4];
...
int AesGetOutLen(int len, int mode) {
  auto rest_len = (unsigned int)(len % AES_BLOCK_SIZE);
  unsigned int padding_len =
      ((AES_ENCRYPT == mode) ? (AES_BLOCK_SIZE - rest_len) : 0);
...
int AesCryptWithKey(const unsigned char *src, unsigned int srclen,
 ...
 if (!src || !AES_KEY || !AES_IV) {
  ...
  AesKeySetup(AES_KEY, key_schedule, AES_KEY_SIZE);

  // 执行加解密计算(CBC mode)
  if (mode == AES_ENCRYPT) {
    AesEncryptCbc(input, src_len, buff, key_schedule, AES_KEY_SIZE, AES_IV);
  } else {
    AesDecryptCbc(input, src_len, buff, key_schedule, AES_KEY_SIZE, AES_IV);
  }
